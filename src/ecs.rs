//! Entity-Component-System: generational arena, sparse-set stores, World,
//! Scene, Input, `GameTime`, and basic physics.
...
impl PhysicsSystem {
    ...
    /// Detects all AABB collision pairs using spatial hash grid (O(n) avg).
    /// The collider AABB is offset by the entity's position.
    #[must_use]
    pub fn detect_collisions(world: &World) -> Vec<CollisionPair> {
        ...
        if entries.len() < 32 {
            // Small N: brute force is faster than hashing overhead
            return Self::brute_force_pairs(&entries);
        }

        // Spatial hash grid broadphase
        let cell_size = 4.0_f64;
        ...
        for cell in grid.values() {
            for (a_idx, &i) in cell.iter().enumerate() {
                for &j in &cell[a_idx + 1..] {
                    let key = if i < j { (i, j) } else { (j, i) };
                    if seen.insert(key) && entries[i].1.intersects(&entries[j].1) {
    ...
    fn brute_force_pairs(entries: &[(EntityId, AABB, u32)]) -> Vec<CollisionPair> {
        let mut pairs = Vec::new();
